	...
	*/
			 					     ...
			 					     GeographicPoint goal, Consumer<GeographicPoint> nodeSearched)
	{
		...
		while (!nodeQueue.isEmpty())
		{
			...
			{
				parentMap.put(node, tempPath);
				if (node.distance(goal)==0)
				{
					parentMap.get(node).add(node);
					GeographicPoint parentNode = parentMap.get(node).get(0);
					while(parentNode.distance(start)!=0) 
					{
						// parent of 
						parentNode = parentMap.get(parentNode).get(0);
						parentMap.get(node).add(0,parentNode);	
					}
	...
	public static void main(String[] args)
	{
		...
		System.out.print(testroute.get(0).toString()+" ");
