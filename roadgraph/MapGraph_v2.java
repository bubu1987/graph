		...
		while(!nodePriorityQueue.isEmpty())
		{
			...
			if (!visitedNodes.contains(current))
			{
				...
				for (GeographicPoint next : adjListsMap.get(current))
				{
					// Hook for visualization.  See writeup.
					nodeSearched.accept(next.getLocation());
					if (next.distance(goal)==0)
					{
						  parentMap.put(next, current);
						  System.out.println("parent :"+current.toString()+" child "+next.toString());
						  retPath.add(next);
						  GeographicPoint parentNode = parentMap.get(next);
						  retPath.add(0,parentNode);	
						  while(parentNode.distance(start)!=0) 
						  {
						     parentNode = parentMap.get(parentNode);
			...
			if (!visitedNodes.contains(current))
			{
				...
				for (GeographicPoint next : adjListsMap.get(current))
				{
				   ...
				   else 
				   {
                       ...
                       if (distanceToDest < distanceToNode.get(next))
					   {   
						   distanceToNode.put(next, distanceToDest);
						   parentMap.put(next, current);
						   nodePriorityQueue.add(next);
					   }
				   }      
	...
	public static void main(String[] args)
	{
		...
		*/
		...
		{	
			...
			System.out.println("This test done.");
		}
		else
		{
	    ...
	    System.out.println(java.util.Arrays.toString(testroute2.toArray()));
	    System.out.println("AStar :");
	    System.out.println(java.util.Arrays.toString(testroute3.toArray()));
